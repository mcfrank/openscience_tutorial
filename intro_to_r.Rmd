...
> ProTip: You can think about `filter`ing as similar to "logical indexing", where you use a vector of `TRUE` and `FALSE`s to get a part of a dataset, for example, `ps_data[ps_data$age > 2,]`. This command creates a logical vector `ps_data$age > 2` and uses it as a condition for filtering.
